...
const CardSongTwo = () => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-5">
      {data.slice(-8).reverse().map((song: any, index: number) => (
        <div
          key={song.id}
        ...
        >
          {/* Izquierda (Número) */}
          <div className="flex-1 bg-gradient-to-br from-accent to-secondary flex items-center justify-center text-white font-black text-3xl shadow-inner group-hover:brightness-110 transition-all">
            {index + 1}
          </div>
          ...
          <div className="flex-[3] p-4 flex flex-col justify-between overflow-hidden">
            <div className="space-y-1">
              <strong className="text-[var(--text-primary)] text-sm font-black block truncate group-hover:text-accent transition-colors">
            ...
            <div className="flex items-center gap-3 text-[9px] font-bold text-[var(--text-secondary)] opacity-50 uppercase tracking-widest">
              ...
              <span className="truncate max-w-[60px]">{song?.categories?.[0]?.name || "Reciente"}</span>
            </div>
          </div>

          {/* Derecha (Acorde principal) */}
          <div className="flex-1 flex items-center justify-center text-accent font-black text-2xl bg-[var(--glass-bg)] border-l border-[var(--glass-border)] group-hover:bg-accent/5 transition-colors">
